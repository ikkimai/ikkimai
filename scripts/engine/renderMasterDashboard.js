 ...
 * Features widescreen layout, Premium Plus Pro colors, and precise SVG animations.
 */
...
function renderMasterDashboard(data) {
  ...
  // Deep Premium Aesthetics Styles
  svg.addStyle(`
    ...
    @keyframes slideUp { 0% { transform: translateY(10px) scaleY(0.1); opacity: 0; } 100% { transform: translateY(0) scaleY(1); opacity: 1; } }
    @keyframes expandBar { 0% { width: 0; } 100% { width: 100%; } }
    @keyframes pulseParticle { 0%, 100% { r: 1.5; opacity: 0.8; } 50% { r: 3; opacity: 1; filter: drop-shadow(0 0 6px rgba(255,255,255,0.8)); } }
    
    .tech-node { animation: floatNode 6s ease-in-out infinite; }
    .anim-bar { transform-origin: bottom; animation: slideUp 1.2s cubic-bezier(0.16, 1, 0.3, 1) backwards; }
    .anim-progress { animation: expandBar 1.5s cubic-bezier(0.16, 1, 0.3, 1) backwards; }
  `);

  // Premium Plus Pro Gradient Definitions
  svg.addDef(`
    <radialGradient id="core-glow" cx="50%" cy="50%" r="50%">
      <stop offset="0%" stop-color="rgba(129, 140, 248, 0.15)" />
      <stop offset="50%" stop-color="rgba(192, 132, 252, 0.05)" />
  ...
  svg.addText("WELCOME TO CREATIVE MIND", { x: HERO_X, y: 220, class: "text-card-title", fill: "#38bdf8" });
  svg.addText("Building digital", { x: HERO_X, y: 290, class: "text-title" });
  svg.addText("experiences", { x: HERO_X, y: 350, class: "text-title" });
  svg.addText("that matter.", { x: HERO_X, y: 410, class: "text-title" });
  
  svg.addText("I design and engineer digital products,", { x: HERO_X, y: 480, class: "text-subtitle" });
  svg.addText("automate workflows and explore", { x: HERO_X, y: 510, class: "text-subtitle" });
  svg.addText("intelligent systems.", { x: HERO_X, y: 540, class: "text-subtitle" });
  ...
  svg.addCircle({ cx: CORE_CX, cy: CORE_CY, r: CORE_R * 2.5, fill: "url(#core-glow)", style: "mix-blend-mode: screen;" });
  ...
  // Generate lines and particles, splitting them into front and back layers for the optical illusion
  for (let i = 0; i < points.length; i++) {
    for (let j = i + 1; j < points.length; j++) {
      ...
      if (d < 50) {
        ...
        const lineStr = `<line x1="${CORE_CX + projI.x}" y1="${CORE_CY + projI.y}" x2="${CORE_CX + projJ.x}" y2="${CORE_CY + projJ.y}" stroke="rgba(192, 132, 252, 0.15)" stroke-width="0.6" />\n`;
  ...
  points.forEach((p, idx) => {
    ...
    const size = Math.max(0.8, proj.scale * 2.5);
    const isFront = p.z > 0;
    const isSpecial = isFront && idx % 12 === 0;
    
    const color = isFront ? premiumColors[idx % premiumColors.length] : "#475569";
    const opacity = isFront ? 0.9 : 0.2;
    const animStyle = isSpecial ? `animation: pulseParticle ${3 + Math.random()*2}s infinite alternate;` : "";
  ...
  // Assemble the core with counter-rotating layers to create a 3D illusion
  const animatedCore = `
    <g>
      ${coreBack}
  ...
  // Orbital Rings using SVG <animateTransform> to fix the top-left origin bug
  const rings = `
    <g>
      <ellipse cx="${CORE_CX}" cy="${CORE_CY}" rx="${CORE_R + 80}" ry="${CORE_R + 15}" fill="none" stroke="rgba(56, 189, 248, 0.15)" stroke-width="1.5" />
    ...
    <g>
      <ellipse cx="${CORE_CX}" cy="${CORE_CY}" rx="${CORE_R + 130}" ry="${CORE_R + 35}" fill="none" stroke="rgba(192, 132, 252, 0.1)" stroke-width="1" />
  ...
  // Ensure we have at least 7 nodes to display around the core
  while(realLangs.length < 7) { realLangs.push("Tech"); }

  // FIX: Use nested <g> to prevent CSS transform (floatNode) from overwriting SVG translate(x,y)!
  const drawNode = (x, y, label, delay) => `
    <g transform="translate(${x}, ${y})">
      <g class="tech-node" style="animation-delay: ${delay}s">
        <circle cx="0" cy="0" r="32" fill="#05050A" stroke="url(#node-border)" stroke-width="1.5" />
        <text x="0" y="4" font-family="${svg.fontUI}" font-size="11px" fill="#e2e8f0" text-anchor="middle" font-weight="600">${label.substring(0, 10)}</text>
      </g>
    </g>
  `;

  const nodes = `
    ${drawNode(CORE_CX - 150, CORE_CY - 140, realLangs[0], 0.2)}
    ${drawNode(CORE_CX + 190, CORE_CY - 110, realLangs[1], 1.5)}
    ${drawNode(CORE_CX - 220, CORE_CY + 10, realLangs[2], 0.7)}
    ${drawNode(CORE_CX + 240, CORE_CY + 60, realLangs[3], 2.1)}
    ${drawNode(CORE_CX - 140, CORE_CY + 170, realLangs[4], 1.1)}
    ${drawNode(CORE_CX + 170, CORE_CY + 190, realLangs[5], 2.8)}
    ${drawNode(CORE_CX, CORE_CY + 240, realLangs[6], 0.4)}
  `;

  svg.addGroup(rings + animatedCore + nodes);
  svg.addText("KNOWLEDGE CORE", { x: CORE_CX, y: 100, class: "text-card-title", "text-anchor": "middle", fill: "#818cf8" });

  // --- RIGHT METRICS (Data Grids) ---
  const stats = data?.stats || { total_commits: 1532, public_repos: 28 };
  
  svg.addGroup(`
    <circle cx="${RIGHT_X}" cy="90" r="16" fill="rgba(56, 189, 248, 0.1)" />
    ...
    <text x="${RIGHT_X + 40}" y="103" font-family="${svg.fontUI}" font-size="12px" fill="#34d399">Online</text>
    
    <text x="${RIGHT_X + 160}" y="86" class="text-sm">Last Updated</text>
    <text x="${RIGHT_X + 160}" y="103" font-family="${svg.fontUI}" font-size="12px" fill="#fff">${new Date().toLocaleDateString('en-US', {month: 'short', day: 'numeric', year: 'numeric'})}</text>
  `);

  // Card 1: Commit Activity
  ...
  for(let col=0; col<22; col++) {
    for(let row=0; row<5; row++) {
      ...
      const opacity = isActivity ? (0.2 + Math.random()*0.8) : 0.05;
      const color = isActivity ? "#818cf8" : "#ffffff"; // Indigo for activity
